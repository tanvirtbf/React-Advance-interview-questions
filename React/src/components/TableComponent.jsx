...
const TableComponent = ({data}) => {
  ...
  const headings = useMemo(()=> {
    return Object.keys(data[0]).map(heading => <th key={heading}>{heading}</th>)
  },[data])

  const tableData = filteredData.map((row)=> <tr key={row.id}>{Object.values(row).map(item=> <td key={item}>{item}</td>)}</tr>)
